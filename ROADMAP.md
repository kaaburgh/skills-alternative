...
The composite must be uniquely most useful against **all** available individual views, so a subject covered by one catalogue is a materially easier win than one covered by three, and how many of each the held-out sample contains is not knowable until the cohort is frozen. `C1`'s threshold is therefore precommitted against a difficulty that varies. It is not corrected for — no stratification is imposed and no minimum comparator count is required, because either would change the sampling design after the fact. Instead the comparator-count distribution of the twenty held-out subjects is reported alongside `C1`, and the decision memo states how it qualifies the claim: a `C1` earned largely on single-comparator subjects is a weaker result than the same number earned against three, and must be described as such rather than as a bare count.
...
Each producer writes a manifest containing input hashes, output hashes, producer command/version, timestamp, and manual interventions. A source capture is labelled `stored`, `externally_immutable`, or `metadata_only`; only the first two are called replayable. The label is checked rather than trusted: `stored` requires the bytes to be present and to reconcile against the recorded hash, and `externally_immutable` requires the coordinate to match a precommitted immutable-form pattern — a commit SHA, a content digest, or an equivalent that cannot silently change — with `SA-004`'s validators enforcing both. A producer that merely writes a label has declared its own compliance, which is the pattern this repository's contract forbids reporting as if it had been checked. Observations are source-faithful extractions, claims normalize propositions and conditions, and cards synthesize claims. A derived layer consumes only frozen hashes from the preceding layer.
...
**`T1` is attested, not verified, and the distinction is stated here because nothing downstream can supply it.** No property of the data can reveal work that was never logged; a complete-looking ledger and a ledger missing an hour are indistinguishable to any validator. `T1`'s assurance comes from contemporaneous timing discipline and from the person doing the work, not from a check, and the Rule 0 condition covering it fires only on unlogged work that is *discovered*. Treat `T1` as the one metric in this experiment whose integrity rests on an honour system, and do not let its appearance beside mechanically-checked metrics imply otherwise.
...
| P1 | Cards whose every decision-relevant factual claim is supported by the cited origin/conditions | 20 cards; at least 19, with zero critical identity/provenance errors; disputed findings enumerated and a sensitivity line reported at P1 ± 1 |
...
`P1` has no middle band: 19 or 20 is required for Keep and anything lower fires the Rule 1 Kill, so a single borderline judgement by a single evaluator separates the two outcomes. The threshold is not softened and no repair path is created — but the evaluator records any finding they judged borderline, the memo enumerates those findings, and the metrics report a sensitivity line at `P1 ± 1`. A decision that would flip on one contested call must be visible as such; it may not be presented as a clean pass or a clean failure.

Report raw counts, the comparator-count distribution of the held-out sample, the initial and corrected identity error rates, publisher/repository/category concentration, one-entity-per-cluster sensitivity, and Wilson intervals for sampled proportions. Thresholds use the integer counts above.

### Decision precedence

Apply the first matching rule:

0. **Kill — experiment invalid, no product inference** if the exact cohort/candidate capture or mandatory comparator capture fails; the held-out subjects become knowable before output freeze; smoke/calibration leaks into an audit; evaluator independence fails; a required denominator is zero/undefined; a held-out quartile holds fewer than five eligible entities; a held-out subject is substituted after the draw; manifests/hashes do not reconcile; a binding output/workflow is replaced or rerun, or binds on a run whose `run_attempt` is not 1; a baseline-completeness defect escapes the frozen validator; the `E1`/`E2` re-derivation cannot be completed; unlogged/reclassified entity-specific work is found (a condition that fires on discovery — see the attestation note at `T1`, since no validator can detect work that was never logged); or a material protocol deviation can affect any threshold.
...
- **Capture-label validation:** The validators check the `stored` / `externally_immutable` / `metadata_only` label against the data rather than accepting it: `stored` must have its bytes present and reconciling against the recorded hash, and `externally_immutable` must carry a coordinate matching the precommitted immutable-form pattern. A capture that merely asserts its label fails.
...
- **Validation / acceptance:** Exactly 500 ranked rows resolve to canonical candidate identities/groups and exactly 100 cohort entities are accepted; every decision has evidence. More than two errors in the 20-entity cohort audit or more than three in the 30-row universe audit requires a full rerun and new pre-evidence freeze, **at most twice**: each rerun is a fresh draw against the same integer bar, so repeating without limit converges on passing by chance rather than by improved canonicalization, and the attempt that happens to pass would become the frozen cohort. A third failure is a substantive identity-layer Kill, not a fourth attempt. Every attempt's error counts are preserved, including the failed ones; more than five unresolved cohort membership ambiguities fires Rule 0. Report initial/corrected error rates and merge/split sensitivity. Smoke/calibration exclusions are committed.
...
- **Next experiment:** Resolve and persist the binding workflow run ID/URL/head SHA/`run_attempt`/`run_started_at`/server `completed_at`, obtain the specified later NIST pulse and persist its chain index, pulse index, `timeStamp`, `outputValue` and signature so the draw is independently replayable rather than merely recorded, derive five eligible subjects per canonical-rank quartile from a pool the exclusions were removed from, randomize equivalent-format views, and create the phased evaluator bundle. For each of the 60 alternative slots the bundle carries target-side identity and job material — canonical identity, source coordinate, and the same decision-relevant field set a baseline view would contain — under the same completeness contract as a baseline view, because the evaluator must judge the slots in Phase 1 from identity and job evidence, before generated rationales are revealed, and without it that judgement degrades into name recognition. Run the frozen completeness validator over every required comparator capture and its baseline view before sealing; do not repair frozen outputs.
